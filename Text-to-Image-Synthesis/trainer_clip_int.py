    ...
    def _train_gan(self, cls):
        ...
        for epoch in range(self.num_epochs):
            for sample in self.data_loader:
                ...
                right_embed = Variable(right_embed.float()).cuda()
                wrong_images = Variable(wrong_images.float()).cuda()
                inter_embed = Variable(inter_embed.float()).cuda()
                ...
                noise_int = Variable(torch.randn(right_images.size(0), 100)).cuda()
                noise_int = noise_int.view(noise.size(0), 100, 1, 1)
                fake_int_images = self.generator(inter_embed, noise_int)
                outputs_int, activation_fake_int = self.discriminator(fake_int_images, right_embed)

                activation_fake = torch.mean(activation_fake, 0)
                activation_real = torch.mean(activation_real, 0)
                activation_fake_int = torch.mean(activation_fake_int, 0)
                ...
                g_loss = criterion(outputs, real_labels) \
                         ...
                         + self.int_coef * criterion(outputs_int, real_labels)
